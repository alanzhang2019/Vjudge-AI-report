# deploy_v3100.ps1 - v3.10.0 一键部署（邮箱注册改造 + short_id 主键）
...
$StagingBase = Join-Path $env:USERPROFILE "_deploy_pkg_v3100"
$TarPath = Join-Path $env:USERPROFILE "luogu-pkg-v3100.tar.gz"
...
scp $TarPath "${Server}:${RemoteDir}/luogu-pkg-v3100.tar.gz"
...
ssh $Server "cd $RemoteDir && chmod +x deploy.sh && ./deploy.sh --from-zip luogu-pkg-v3100.tar.gz"
if ($LASTEXITCODE -ne 0) { Write-Error "部署失败"; exit 1 }

Write-Host "`n[$(Get-Date -Format 'HH:mm:ss')] ✅ v3.10.0 部署完成" -ForegroundColor Green
