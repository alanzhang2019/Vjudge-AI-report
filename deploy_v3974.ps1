# deploy_v3974.ps1 - v3.9.74 一键部署（VJudge 跨平台数据接入,取代 AtCoder）
...
$StagingBase = Join-Path $env:USERPROFILE "_deploy_pkg_v3974"
$TarPath = Join-Path $env:USERPROFILE "luogu-pkg-v3974.tar.gz"
...
scp $TarPath "${Server}:${RemoteDir}/luogu-pkg-v3974.tar.gz"
...
ssh $Server "cd $RemoteDir && chmod +x deploy.sh && ./deploy.sh --from-zip luogu-pkg-v3974.tar.gz"
if ($LASTEXITCODE -ne 0) { Write-Error "部署失败"; exit 1 }

Write-Host "`n[$(Get-Date -Format 'HH:mm:ss')] ✅ v3.9.74 部署完成" -ForegroundColor Green
