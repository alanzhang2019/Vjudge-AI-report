...
⚠️ v3.10.0 · 此测试已被 test_v3100_register.py 取代（注册入口改为邮箱 + 密码）。
         本文件仅作历史参考,不再跑实际断言（直接 sys.exit(0) 跳过）。
         旧学员仍可通过 admin 后台用 luogu_uid 注册（/admin/students/new）。
...
执行：python test_v352_register.py  →  v3.10.0 起直接 0 退出
"""
import sys
print("[v3.10.0] test_v352_register.py 已被 test_v3100_register.py 取代,跳过。")
sys.exit(0)


# ----- 以下代码保留作历史参考,v3.10.0 起不再执行 -----
# ruff: noqa: E800
if False:
    import time
